...
function updateBookCopy(id, bookCopyData) {
  ...
  if (borrowerId && !getUserById(borrowerId)) {
    throw new Error(`BookCopy needs valid or empty borrowerId '${borrowerId}'`);
  }
...
function createUser(userData) {
  ...
  if (!info || info.lenght < 1) {
    throw new Error("User needs valid info");
  }
  const color =
    VALID_AVATAR_COLORS[Math.floor(Math.random() * VALID_AVATAR_COLORS.length)];
